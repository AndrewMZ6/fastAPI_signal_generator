...
def generate_ofdm_nopilots(fftsize: int, M_order: int) -> np.ndarray:
    ...
    M = cp.modulation.TQAMModem(M_order)
...
def generate_ofdm_withpilots(fftsize: int, M_order: int, pilots_num: int) -> tuple:
    ...
    # modulate
    M = cp.modulation.TQAMModem(2**mu)
    modBits = M.modulate(bits)
...
def ofdm_fft_morder(fftsize: int, M_order: int) -> np.ndarray:
    gsize = 100
...
def ofdm_fft_morder_bw_fs_fc(fftsize: int, Modulation_order: int, BW: float, fs: float, fc: float):
    arr = ofdm_fft_morder(fftsize, Modulation_order)
